...
from miw.schema import (Claim, Dependency, Finding, ProbeResult, ResearchResult,
                        UncitedClaim, utcnow)
    ...
    def ensure(signal: str) -> Finding:
        if signal not in out:
            ...
            out[signal] = Finding(
                ...
                question_ids=[l.content_id for l in
                              (dep.questions_that_execute_it +
                               dep.questions_that_mention_it)[:8]],
                raised_at=utcnow())
        return out[signal]
            ...
            for change in probe.declared_changes:
                ...
                try:
                    f.claims.append(Claim.build(
                        ...
                        statement=(f"n8n declares: {change.get('title')} "
                                   f"(n8n {change.get('n8n_version')})"),
                        source_url=url, quote=quote, subject=dep.subject()))
                except UncitedClaim:
                    pass
        if probe.status == "broken" and "S1" not in out and "S4" not in out:
            f = ensure("S1")
...
def recommend(dep: Dependency, f: Finding) -> str:
    ...
    if f.alternatives:
        ...
        alt_txt = (f" Candidate replacement: {a.name} ({a.homepage}) - {free}"
    ...
    base = {
        ...
        "S7": f"{dep.canonical_name} may be retired - update the model id used in "
              f"the session and re-run its examples.",
